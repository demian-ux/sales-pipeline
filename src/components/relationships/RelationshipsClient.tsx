...
import type { Lead, Company, Opportunity, Interaction, Campaign } from '@/lib/types'
...
export default function RelationshipsClient({ leads, companies, opportunities, campaigns }: Props) {
  ...
  // ── Grouping (unchanged from server version) ──────────────────────────────
  const { groups, unassigned } = useMemo(() => {
    ...
    const rest = leads.filter((l) => !assignedIds.has(l.lead_id))
    return { groups: grouped, unassigned: rest }
  }, [leads])

  // ── Selection helpers ─────────────────────────────────────────────────────
  const allLeadIds = useMemo(() => leads.map((l) => l.lead_id), [leads])
        ...
        <div style={{
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none' }}>
            ...
            <span style={{ color: someSelected ? 'var(--accent)' : 'var(--text-muted)', fontWeight: someSelected ? 500 : 400 }}>
              {someSelected ? `${selected.size} selected` : `Select all (${leads.length})`}
            </span>
          </label>
...
function LeadRow({
  lead, company, openOpps, isSelected, onToggle, compact,
}: {
  lead: Lead
