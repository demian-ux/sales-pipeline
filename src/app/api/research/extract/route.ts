...
import { getLeadById, getCompanyById, getResearchForLead } from '@/lib/sheets'
...
export async function POST(req: Request) {
  try {
    ...
    if (lead_id) {
      ...
      // Provide minimal context without a full lead record
      lead = company_name || lead_name
        ? { full_name: lead_name ?? '', company_name: company_name ?? '', title: '', pipeline_stage: 'New Lead' } as Parameters<typeof extractResearchSignals>[1]
        : null
    }
    ...
    console.error('POST /api/research/extract error:', err)
